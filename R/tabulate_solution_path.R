...
tabulate_solution_path <- function(x,
  ...
  # If there is only one solution path, remove gamma and esp from summary
  if(length(x) == 1) {
    mod_sum_list[[1]]$gamma <- NULL
    mod_sum_list[[1]]$eps <- NULL
  }
  ...
  if(!is.null(x[[1]]$bls) & is.na(full_rss)) {
    ...
    # If there is only one solution path, remove gamma and eps from summary
    if(length(x) == 1) {full_row <- full_row[-c(1:2)]}

    mod_sum_df[nrow(mod_sum_df) + 1,] <- full_row
