...
deviation in random effects models, all $\binom{T_i}{2}$ pairwise contrasts in 
multi-arm trials comparing $T_i$ treatments must be inputted (e.g., 3 contrasts 
for 3-arm trials, 6 contrast for 4-arm trials, etc.). 
...
subnetwork, meaning there is a single connected network.
...
treatment estimated in each study, 2) the corresponding variance-covariance 
...
solved based on a dataset that only included the character names, we need to 
...
You may re-run the NMA based on the new treatment codes. Here, we will run a 
